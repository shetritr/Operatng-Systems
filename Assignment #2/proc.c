...
found:
  ...
  *(uint*)sp = (uint)trapret;
  ...
  for(frame = p->cstack.frames ;  frame < (p->cstack.frames + 10); frame++){
      frame->used = 1;
  }
  return p;
}
...
int
push(struct cstack *cstack, int sender_pid, int recepient_pid, int value) {//push signal to panding queue
